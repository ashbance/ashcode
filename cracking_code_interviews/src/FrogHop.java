 ...
 * river (position 0) and wants to get to the opposite bank (position X+1). Leaves fall from a tree onto the surface of the river.
 ...
 You are given a zero-indexed array A consisting of N integers representing the falling leaves. A[K] represents the position
 ...
 cross only when leaves appear at every position across the river from 1 to X (that is, we want to find the earliest
 moment when all the positions from 1 to X are covered by leaves). You may assume that the speed of the current in the river is
...
public class FrogHop {
    ...
    private static int solution(int[] A, int x) {
        ...
        int[] checkList = new int[x];  //x=5,  0,1,2,3,4
        int count = 0;
        for (int i=0; i<checkList.length; i++) { checkList[i] = -1;}


        for (int t=0; t<A.length; t++) {
            int pos = A[t];

            if (pos > checkList.length) continue;

            if (checkList[pos -1] == -1) {
                checkList[pos - 1] = t;
                count++;
            }
